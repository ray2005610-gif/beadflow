...
export function RecognitionReviewPanel({ grid, threshold, onSelectColor }: { grid: PatternGrid; threshold: number; onSelectColor: (code: string) => void }) {
      ...
      <div className="review-list">
        {cells.map((cell) => (
          <button key={`${cell.row}-${cell.col}`} onClick={() => onSelectColor(cell.colorCode)}>
            ...
            <span>信心 {Math.round((cell.confidence ?? 0) * 100)}% / 原色 {cell.rawHex ?? "-"} / MARD {cell.hex}</span>
            <span>H {cell.rawHue == null ? "-" : Math.round(cell.rawHue * 360)} / S {cell.rawSaturation == null ? "-" : cell.rawSaturation.toFixed(2)}</span>
            <span>候選：{cell.candidates?.map((item) => `${item.code} ${Math.round(item.adjustedDistance ?? item.distance)} H${item.hue == null ? "-" : Math.round(item.hue * 360)}`).join("、") ?? "-"}</span>
          </button>
        ))}
      </div>
    </div>
