...
self.onmessage = (event) => {
  ...
  try {
    ...
    const correctionPalette = localPalette.length ? localPalette : palette;
    const grid = profile.time("legendValidation",()=>legend?.length ? applySafeCorrections(detected,legend,correctionPalette) : detected);
