...
try {
  ...
  await page.reload();
  await page.locator('.bf-desktop-nav').getByRole('button',{name:'照片轉拼豆',exact:true}).click();
  await expect(page.getByRole('heading',{name:'照片轉拼豆',exact:true})).toBeVisible();
  ...
  const canvasImage=()=>patternCanvas.evaluate(canvas=>canvas.toDataURL());
  const initialCanvas=await canvasImage();
  await page.getByRole('button',{name:'格線',exact:true}).click();await page.waitForTimeout(80);
  assert.notEqual(await canvasImage(),initialCanvas,'grid toggle changes the rendered canvas');
  await page.getByRole('button',{name:'格線',exact:true}).click();
  await page.getByRole('button',{name:'色號',exact:true}).click();await page.waitForTimeout(80);
  assert.notEqual(await canvasImage(),initialCanvas,'symbol toggle changes the rendered canvas');
  await page.getByRole('button',{name:'色號',exact:true}).click();
  await page.getByRole('button',{name:'水平鏡射',exact:true}).click();await page.waitForTimeout(80);
  assert.notEqual(await canvasImage(),initialCanvas,'horizontal mirror changes cell positions');
  await expect(page.getByRole('button',{name:'還原原圖',exact:true})).toBeEnabled();
  await page.getByRole('button',{name:'還原原圖',exact:true}).click();await page.waitForTimeout(80);
  assert.equal(await canvasImage(),initialCanvas,'restore returns to the original grid');
  await page.getByRole('button',{name:'進入修正模式',exact:true}).click();
  await expect(page.getByText('目前為修正模式，點擊格子會修改圖紙顏色。',{exact:true})).toBeVisible();
  await expect(page.getByRole('button',{name:'畫筆',exact:true})).toBeVisible();
  await page.getByRole('button',{name:'離開修正模式',exact:true}).click();
  await expect(page.getByRole('button',{name:'畫筆',exact:true})).toHaveCount(0);
  await page.getByRole('button',{name:'展開色卡',exact:true}).click();
  assert.equal(await page.evaluate(()=>localStorage.getItem('paletteCollapsed')),'false');
  await expect(page.getByPlaceholder('搜尋色號')).toBeVisible();
  await page.getByRole('button',{name:'收合色卡',exact:true}).click();
  assert.equal(await page.evaluate(()=>localStorage.getItem('paletteCollapsed')),'true');
  await page.locator('.stat-row').first().click();await page.waitForTimeout(80);
  await expect(page.getByRole('button',{name:'只看未完成',exact:true}).first()).toBeEnabled();
  assert.notEqual(await canvasImage(),initialCanvas,'statistics selection highlights the canvas');
  await page.getByRole('button',{name:'取消高亮',exact:true}).click();
  const inventorySection=page.locator('.accordion-trigger').filter({hasText:'庫存管理'});
  await inventorySection.click();
  await page.getByLabel('數量',{exact:true}).fill('1000');
  await page.getByRole('button',{name:'批次套用',exact:true}).click();
  await expect(page.getByText(/已將 2 個顏色設定為 1000 顆/)).toBeVisible();
  await page.getByRole('button',{name:'進入製作模式',exact:true}).click();
  await expect(page.getByRole('button',{name:'離開製作模式',exact:true})).toBeVisible();
  await page.getByRole('button',{name:'專注製作',exact:true}).click();
  await expect(page.locator('.editor-layout')).toHaveClass(/bf-focus-mode/);
  await page.getByRole('button',{name:'退出專注製作模式',exact:true}).click();
  await page.getByRole('button',{name:'離開製作模式',exact:true}).click();
  ...
  await page.getByRole('button',{name:'確認圖例並開始辨識',exact:true}).click();
  ...
  // Photo controls: size, fit, comparison, mask history, and the unavailable-provider state all affect real output/UI.
  await page.getByRole('button',{name:'返回入口',exact:true}).click();
  ...
  const photo=await page.evaluate(()=>{
    ...
    ctx.fillStyle='#f3d2bd';ctx.fillRect(0,0,80,40);ctx.fillStyle='#8f392f';ctx.fillRect(20,8,40,24);return canvas.toDataURL();
  });
  await page.locator('input[type=file]').first().setInputFiles({name:'photo.png',mimeType:'image/png',buffer:Buffer.from(photo.split(',')[1],'base64')});
  await expect(page.getByText('52 × 26',{exact:true})).toBeVisible({timeout:10000});
  const fitSelect=page.locator('.side-rail .panel').filter({hasText:'圖片適配'}).locator('select').first();
  await fitSelect.selectOption('stretch');
  await expect(page.getByText('52 × 52',{exact:true}).last()).toBeVisible({timeout:10000});
  await fitSelect.selectOption('contain');
  await page.locator('.preview-toolbar select').selectOption('original');
  await expect(page.locator('.preview-panel figure')).toHaveCount(1);
  await page.locator('.preview-toolbar select').selectOption('compare');
  await expect(page.locator('.preview-panel figure')).toHaveCount(2);
  await page.getByRole('button',{name:'AI Q 版後轉拼豆',exact:true}).click();
  await expect(page.getByText('AI Q 版服務尚未設定；原圖仍可直接轉拼豆。',{exact:true})).toBeVisible();
  await expect(page.getByRole('button',{name:'生成 Q 版預覽',exact:true})).toHaveCount(0);
  await page.getByRole('button',{name:'改用原圖直接轉拼豆',exact:true}).click();
  await page.getByRole('button',{name:'清除選取',exact:true}).click();
  await expect(page.getByText(/目前保留 0 格，透明 2,704 格/)).toBeVisible();
  await page.getByRole('button',{name:'復原',exact:true}).click();
  await expect(page.getByText(/目前保留 2,704 格，透明 0 格/)).toBeVisible();
  await page.getByRole('button',{name:'產生可編輯圖紙',exact:true}).click();
  await expect(page.getByRole('button',{name:'儲存圖紙',exact:true})).toBeVisible({timeout:10000});

  // Manual drawing controls: fill, eyedropper, eraser, palette selection, and replace update saved grid data.
  await page.getByRole('button',{name:'返回入口',exact:true}).click();
  await page.locator('.bf-desktop-nav').getByRole('button',{name:'手動畫圖紙',exact:true}).click();
  await expect(page.getByRole('heading',{name:'手動畫圖紙',exact:true})).toBeVisible();
  await page.getByRole('button',{name:/^小掛件 16×16/}).click();
  await page.getByRole('button',{name:'建立空白圖紙',exact:true}).click();
  const manualCanvas=page.locator('canvas.pattern-canvas');
  const box=await manualCanvas.boundingBox();assert.ok(box);
  const center={x:box.width/2,y:box.height/2};
  await page.getByRole('button',{name:'油漆桶',exact:true}).click();
  await manualCanvas.click({position:center});
  await page.getByRole('button',{name:'儲存圖紙',exact:true}).click();
  let manual=await page.evaluate(()=>JSON.parse(localStorage.getItem('beadflow:projects')).find(p=>p.sourceType==='manual_drawing'));
  assert.equal(manual.grid.flat().filter(cell=>!cell.empty).length,256,'fill updates every connected empty cell');
  await page.getByRole('button',{name:'橡皮擦',exact:true}).click();
  await manualCanvas.click({position:center});
  await page.getByRole('button',{name:'儲存圖紙',exact:true}).click();
  manual=await page.evaluate(()=>JSON.parse(localStorage.getItem('beadflow:projects')).find(p=>p.sourceType==='manual_drawing'));
  assert.equal(manual.grid.flat().filter(cell=>!cell.empty).length,255,'eraser clears a cell');
  await page.getByRole('button',{name:'取色',exact:true}).click();
  await manualCanvas.click({position:{x:Math.max(10,box.width*.25),y:Math.max(10,box.height*.25)}});
  await expect(page.getByRole('button',{name:'畫筆',exact:true})).toHaveClass(/active/);
  await page.getByRole('button',{name:'展開色卡',exact:true}).click();
  await page.locator('button[title^="A2 "]').first().click();
  await page.getByRole('button',{name:'替換色號',exact:true}).click();
  await manualCanvas.click({position:{x:Math.max(10,box.width*.25),y:Math.max(10,box.height*.25)}});
  await page.getByRole('button',{name:'儲存圖紙',exact:true}).click();
  manual=await page.evaluate(()=>JSON.parse(localStorage.getItem('beadflow:projects')).find(p=>p.sourceType==='manual_drawing'));
  assert.ok(manual.grid.flat().filter(cell=>!cell.empty).every(cell=>cell.colorCode==='A2'),'replace color updates all matching cells');

  mkdirSync('work',{recursive:true});writeFileSync('work/after-results.json',JSON.stringify(result,null,2));
  assert.deepEqual(errors,[]);
  console.log(JSON.stringify(result,null,2));
  console.log('PASS: worker, OCR, legend restriction, UI controls, photo flow, manual tools, save, export, mobile, no console/runtime errors');
} finally {
